...
Box::Box(Graphics& gfx, std::vector<std::shared_ptr<Bindable>> binds, glm::mat4 transform) : transform(transform)
{
	for (auto b : binds)
	{
		AddBind(b);
...
glm::mat4 Box::GetTransformXM() const
{
	return transform;
}

glm::mat4 Box::GetInitTransform() const
{
